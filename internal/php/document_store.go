...
func (s *DocumentStore) contextSnapshot() (config.Psr4Map, string) {
	s.mu.Lock()
	defer s.mu.Unlock()
	return s.psr4, s.root
}
...
func (s *DocumentStore) Configure(psr4 config.Psr4Map, workspaceRoot string) {
	s.mu.Lock()
	s.psr4 = psr4
	...
	for _, entry := range entries {
		if entry != nil && entry.doc != nil {
			configureDocumentContext(entry.doc, entry.path, psr4, workspaceRoot)
		}
	}
}
...
func (s *DocumentStore) RegisterOpen(path string, doc *Document) {
	...
	psr4, root := s.contextSnapshot()
	configureDocumentContext(doc, path, psr4, root)
...
func configureDocumentContext(doc *Document, path string, psr4 config.Psr4Map, workspaceRoot string) {
	if doc == nil {
		return
	}
	if path != "" {
